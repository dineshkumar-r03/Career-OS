...
const CareerMentorPage = () => {
  ...
  const handleSend = async (customPrompt) => {
    ...
    if (!promptToSend.trim() || streaming || !activeSessionId) return;

    setInput('');
    setStreaming(true);

    // 1. Add User message to local UI state immediately
    ...
    let accumulated = '';
    const url = mentorService.getStreamChatUrl(activeSessionId, promptToSend);
    ...
    source.onmessage = (e) => {
      ...
      // Update latest AI message text token-by-token
      setMessages(prev => {
        const updated = [...prev];
        if (updated.length > 0) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        <div className={`lg:col-span-${recommendedStories.length > 0 ? '2' : '3'} flex flex-col glass-card min-h-[580px] relative animate-slide-up delay-75`}>
          ...
          <div className="p-4 border-b border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
            ...
            {streaming && (
              <span className="text-[10px] text-indigo-500 font-bold animate-pulse">Streaming reply...</span>
            )}
          </div>
          ...
          <div className="p-4 border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/30 dark:bg-slate-900/10 rounded-b-2xl">
            <div className="flex gap-2">
              <input
                ...
                disabled={!activeSessionId || streaming}
                placeholder={activeSessionId ? "Ask a career question..." : "Create a session to begin..."}
          ...
          <div className="lg:col-span-1 glass-card p-4 space-y-4 animate-slide-up delay-100">
            ...
            <div className="space-y-3 max-h-[480px] overflow-y-auto pr-1">
              {recommendedStories.map((story) => (
                ...
                >
                  <span className="inline-block px-2 py-0.5 bg-indigo-50 dark:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400 rounded-md text-[9px] font-bold uppercase mb-2">
